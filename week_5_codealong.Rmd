...
penguin_f_summary <- penguin_f %>% 
  group_by(species) %>% 
  summarize(
    mean_flm = mean(flipper_length_mm),
    sd_flm = sd(flipper_length_mm),
    sample_size = n(),
    se_flm = sd(flipper_length_mm)/sqrt(n()),
    variance_flm = var(flipper_length_mm)
    )
```
...
ggplot() +
  geom_beeswarm(data = penguin_f, 
                ...
                size = 3,
                alpha = 0.5) +
  geom_point(data = penguin_f_summary, aes(x = species, y = mean_flm),
             size = 4) +
  geom_errorbar(data = penguin_f_summary,
                aes(x = species,
                    ymax = mean_flm + sd_flm,
                    ymin = mean_flm - sd_flm),
                width = 0.1) +
  theme_light()
```
